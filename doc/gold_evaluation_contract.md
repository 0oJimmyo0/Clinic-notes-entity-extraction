...
| Primary treatment-relevant target versus broader named-mention sensitivity; class-only/allergy/uncertain rules | **OPEN** |
...
The guide must give examples and a prespecified in-scope/excluded decision for each of: current, historical, planned/discussed, negated, allergy-only, family-member/general-discussion, class-only/vague, combination, brand, dose-only/anaphoric, and repeated mentions. The [legacy task-scope audit](legacy_task_scope_audit.md) shows action/context-triggered candidate selection, **not** exhaustive drug-only scanning across all note text. Provisional endpoint hierarchy: (1) **primary treatment-relevant named evidence** tied to current use, an explicit treatment action, a firm plan, or an explicit taking/not-taking statement; (2) **broader patient-linked named mentions**, including remote historical evidence, as a prespecified sensitivity; (3) separate clinical and identity-resolvability strata; and (4) excluded allergy-only/family/general-discussion text. The clinician must decide context-poor lists, tentative consideration, and negated statements explicitly—negation is not automatically excluded. Choose the boundary on development data before new gold scoring. Do not silently include class-only or uncertain identity mentions in ingredient-level scoring or remove difficult mentions after scoring.
...
5. **Hierarchical targets, ambiguous/class-only/uncertain gold and predictions:** freeze membership in primary treatment-relevant and broader named-mention targets **from note evidence, blinded to system output**. Freeze separate detection and normalized-identity scoring masks, including whether predictions overlapping secondary-but-not-primary or excluded/unresolvable mentions are ignored or count as primary false positives. Report target-specific gold/prediction denominators, count/proportion excluded from ingredient scoring, and where useful both full-sample joint yield and conditional performance. No post hoc removal or unexplained denominator change.
...
7. **Candidate recall:** every gold mention in the **primary treatment-relevant target** is the primary denominator, including candidate-negative notes. Separately report recall on the broader named-mention target. Match against the *candidate stage before normalization*, using its own frozen span rule. Report candidate-negative and candidate-positive note misses and important clinical strata (current, historical, planned, negated) when adequately powered.
