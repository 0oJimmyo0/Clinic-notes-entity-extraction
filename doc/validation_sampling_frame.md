...
Use the [PHI-free investigator batch attestation template](investigator_exposure_attestation_template.md) to document what each original investigator actually viewed or used by artifact/run. Do **not** require 1,275 individual attestations when verified batch coverage and a restricted ID list can establish the same fact; investigate exceptions or untraceable exposure individually. The analysis team then classifies each restricted matrix row as **A (no known exposure), B (automated-only), C (human inspected), D (development-informing), or Unknown**, with source/attestor/date and a decision reason. B is eligible in principle; C/D should be excluded from pristine gold; Unknown remains ineligible until resolved. Preserve the provisional class separately from the confirmed class. A blanket or unsigned assertion is not sufficient to turn all provisional B rows into confirmed B. Do not use the pilot reserve until governance and classification pass.
...
5. Collect batch-level attestations covering automated runs, completed review files, manual alias sources, off-file review, and all relevant investigators; map verified artifact-level claims and patient-linked exceptions to the existing restricted **provisional** matrix. Record confirmed A/B/C/D/Unknown classifications, source attestations, reason codes, and an aggregate exclusion flow. The ID-level register and any note text must remain outside public/manuscript/Overleaf repositories.
