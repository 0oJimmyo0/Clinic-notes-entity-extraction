...
1. **RQ1 — Independent validity:** How accurately does the frozen note-side pipeline identify and normalize medication evidence in previously unused raw clinic notes? Primary evidence: candidate recall; end-to-end normalized precision, recall, and F1 with patient-clustered intervals; same-gold comparators.
...
|---|---|---|
| Constructed reference and candidate ceiling | Blinded raw-note gold; measure all gold mentions and candidate misses | 27,752-row resource for large-cohort characterization |
...
**Independent-sample feasibility check (updated 2026-09-24):** `episode_notes/manifests_clinic_only/evaluation_note_manifest.csv` lists 2,029 patients. Of these, 1,276 are outside the 761-patient BIBM cohort, with 23,211 clinic-like note rows. The new [ID-only sampling-frame audit](validation_sampling_frame.md) shows that **1,261 of these 1,276 patients also appear in prior automated selection/packet artifacts**; only 15 are absent from all selected packet/review artifacts. Packet presence does not itself prove human review or model-development leakage, but the pool is **not certified untouched**. Full-text parquet keys exist for all 23,211 notes, which does not prove usable/full/authorized text. Investigators must classify prior exposure and governance before any pilot or gold draw. Conversely, only five BIBM-cohort patients are outside the prior 606/150 patient split, so the existing BIBM test patients cannot furnish 75--100 genuinely untouched patients. Treat prior BIBM test results as retrospective characterization, not pristine independent validation.
...
6. Verify that every reported result can be regenerated without absolute `/Users/...` paths. Freeze candidate rules, lexical cleanup, base aliases, development-only aliases, action rules, terminology versions, and evaluator **before gold-test scoring**. Record each resource's creation date, source patients, allowed use, hash, and code commit. No gold-test inspection may feed back into this locked primary run.
...
Annotators see the raw note only: no Qwen output, medspaCy/pipeline predictions, candidate list, projected labels, or structured medication history. Create immutable note IDs and a blinded annotation export. Use a minimal ontology:

- **Required primary labels:** mention validity, exact span/local offsets, canonical medication identity (ingredient-level and RxNorm-linked where resolvable), and an explicit unresolvable/uncertain identity code.
- **Secondary labels:** action (start, stop, hold, continue, change, discussed/other, unclear) and temporality (current, planned, historical, unclear), where feasible.
- **Optional/contextual labels:** negation, certainty, reviewer confidence, and rationale unless needed to decide mention validity.

The pilot/guide must settle whether historical, negated, discussed, and class-only mentions are in the primary target. This boundary must be compatible with what the frozen pipeline was built to identify and be recorded in `doc/gold_evaluation_contract.md` before gold scoring. Clinical correspondence with structured EHR is reviewed later, separately.

Double-code 20--30% independently, including examples from every predeclared stratum, then adjudicate disagreements. Revise the guide using the **separate pilot reserve or development notes**, then freeze it before gold-test annotation. Report span agreement/F1, canonical-label agreement, and action-label kappa or an appropriate prevalence-aware alternative. Correspondence with structured history is a **separate later review**, not part of the blinded note-gold annotation. If action or temporality agreement is low, qualify or remove those claims rather than silently recoding the gold set after scoring.

### Gold-set metrics

Report a sequential error cascade:

1. candidate-generation recall among *all human gold mentions*, explicitly separating misses in candidate-negative and candidate-positive notes;
2. span validity;
...
| 2 | **In progress:** ID-only aggregate audit is complete; investigators must now classify selection/packet exposure versus actual manual inspection or tuning, confirm governance, and verify permitted full-note content access. | `resources/doc/validation_sampling_frame.md` has preliminary aggregate counts; restricted ID-level eligibility register remains to be created by authorized investigators. | At least 75--100 genuinely uninspected eligible patients with 300--500 usable raw notes; otherwise acquire a new approved slice and revise the independence claim. Current gate: **not passed**. |
| 3 | Audit existing numbers/resources and prepare `resources/doc/metric_ledger.md` and `resources/doc/resource_chronology.md`. | Source/denominator ledger and dated provenance. | Every reused BIBM number traceable; no new-test patient contributed to a primary alias/rule. |
...
- revised annotation guideline and template;
