...
The code audit explains the system's historical scope; it does **not** establish clinical relevance by itself. A qualified clinician must supply the clinical rationale and boundary examples for the treatment-relevant target. Keep the broader sensitivity even if its recall is poor.
...
5. **Bounded LLM-bootstrap robustness:** Qwen-assisted labels belong to the historical large-scale reference-construction process, **not** to the independent RQ1 gold truth. A multi-LLM benchmark is not a submission gate. If approved and computationally feasible, compare the original Qwen version with one alternative on a bounded development or separate sensitivity sample using the same schema; do not use new gold-test notes to tune or reconstruct truth. This remains sensitivity analysis, not a new gold standard or model leaderboard.
...
For the ordered, evidence-based completion checklist, use [JAMIA Open submission checklist](jamia_open_submission_checklist.md); this plan supplies the full methods and rationale.
...
The study uses clinical records. Obtain the **actual** Vanderbilt/VUMC IRB or non-human-subjects determination and verify whether consent was obtained, waived, or not applicable before writing the ethics statement. This is a submission gate, not a wording exercise. Describe the restricted data and access conditions accurately; publish code or aggregate artifacts only after PHI/security review. Disclose the LLM's role in historical reference construction and any manuscript/analysis AI use per journal policy. The [journal instructions](https://academic.oup.com/jamiaopen/pages/general_instructions) require related **published** materials, including conference proceedings if any, for overlap assessment; identify the arXiv preprint and any related BIBM material accurately in the cover letter and explain the substantive new validation. Do not imply BIBM acceptance or proceedings publication if the submission was rejected, or that the journal submission is a transfer.

### Submission-readiness gates (status as of 2026-09-24)

This is a **design-ready, not evidence-ready** protocol. No item below becomes complete merely because a document describes how it will be done.

| Gate | Current status | Evidence required to close it |
|---|---|---|
| Governance and independent sample | **OPEN**; zero patients certified eligible | PI/data-owner authorization for note access/annotation; verified investigator batch attestations and restricted patient-level exceptions; at least 75--100 eligible, uninspected/development-independent patients or an approved new slice. |
| Clinical task definition | **OPEN**; guide provisional | Qualified clinician's signed-off primary/broader/excluded rules with clinical rationale and boundary examples; second independent annotator designated. |
| Provenance and pre-test freeze | **OPEN**; metric ledger/resource chronology absent | Reconcile BIBM denominator and held-out versus full-resource values; record source/resource versions, input view, sampling/evaluator/FP rules, and frozen hashes before final gold draw/scoring. |
| Independent RQ1 results | **NOT STARTED** | Authorized pilot, then blinded raw-note gold with double coding/agreement, candidate recall, end-to-end normalized P/R/F1, design-aware patient-clustered intervals, and same-gold comparator results. |
| RQ3 correspondence calibration | **NOT STARTED** | Semantic-by-temporal estimates, human review of relaxed matches, and patient-shuffled null; weaken claims if broad/late matches are not clinically or statistically persuasive. |
| Manuscript integrity and fit | **PENDING RESULTS** | Rewrite around new gold rather than the constructed reference; label Qwen-derived resource and conditional BIBM metrics correctly; substantiate ethics/data availability, disclose preprint/related materials, and meet journal format after results are known. |

If the first three gates cannot close, do not draw or score a new gold set. If RQ1 or RQ3 does not support the proposed interpretation, narrow the claims rather than treating the original BIBM results as a substitute. Same-institution replication and extra LLMs are conditional, not submission gates.
