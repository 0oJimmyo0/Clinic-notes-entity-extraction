...
| **2. [Provisional raw-note annotation guide](raw_note_annotation_guide_draft.md)** | Review every proposed inclusion/exclusion rule and orthogonal field. For historical, planned, negated, copied-list, allergy-only, and class-only mentions, decide **primary treatment-relevant / broader secondary only / excluded**, with rationale. Synthetic examples are starting points, not signed-off rules. | A marked-up guide or section-specific accept/change/keep-open decisions, plus missing synthetic examples. |
| **3. [Gold evaluation contract](gold_evaluation_contract.md)** | Check the primary and broader-sensitivity **denominators** (especially class-only/unresolvable mentions), prediction false positives on non-primary text, ingredient-set identity, and span matching. Mark what must remain open until the pilot; do not fill `OPEN` fields by guessing. | Proposed primary/secondary endpoint boundaries and unresolved scoring decisions in the review form. |
...
| **7. [Legacy pipeline task-scope audit](legacy_task_scope_audit.md)** | Check whether the code-derived description of action/context-triggered candidate selection matches the clinical task the new primary endpoint should test. This is not a request to excuse candidate misses. | Accept/change/keep-open decision on the proposed treatment-relevant primary target, plus examples at its boundary. |
...
- **Original investigators/developers:** identify who viewed old packets/notes, link alias resources to source patients where possible, and use the separate [PHI-free batch attestation template](investigator_exposure_attestation_template.md) to document whether candidate patients informed prompts, aliases, rules, thresholds, or model choices.
